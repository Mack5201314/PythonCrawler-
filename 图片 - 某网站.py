...
def set_IPlsit(url):
    ...
    proxy_support = ure.ProxyHandler({'http':'119.6.144.73:81'})
    opener = ure.build_opener(proxy_support)
    # 安装opener
    ...
    # req = ure.Request(url,headers)
    # response = ure.urlopen(req)
    # html = response.read().decode('utf-8')
    # print(html)


def IPspider(numpage, headers):
    ...
    for num in range(1,numpage+1):
        ...
        # 获取网页内容
        request=ure.Request(ipurl,headers=headers)
        content=ure.urlopen(request).read()
        # 创建BeautifulSoup对象
...
# 下载 seed_url 网页的源代码
def download(seed_url):
    headers = set_user_agent()
    request = ure.Request(seed_url,headers)
...
""" 功能：找出我们需要的url列表
（1、满足我们需要的url格式 2、不能存在重复的url）"""

# 下载图片...


# 调用程序
# print(set_user_agent())
ip = set_IPlsit()